...
// (T)->      RUN ->|  <BL_run>,permill  | notify run monitoring record
//                  |....................|
...
//                  |....................|
//                  |        SYS:        | SYS input interface
// (A)<-      RUN <-|  <BL_run>,permill  | notify run monitoring record
//                  +--------------------+
//                  |      <OTHER>:      | <OTHER> input/output interface
// (U)->        * ->|        ...         | any other input event message will
...
//==============================================================================
  ...
  {
    static BL_oval A = app;       // all but SYS: msg's to be forwarded to app

    switch (bl_cl(o))             // super fast dispatching
    {
      case _SYS:                  // SYS: interface calls must not be forwarded
        if (bl_op(o) == INIT_)
        {
          LOG(2,BL_B "init bl_top");
          A = bl_cb(o,(A),"bl_top:(A)");
        }
        else if (bl_op(o) == RUN_)
          bl_fwd(o,val,(A));      // emit the run log message to app
        return 0;

      default:
        return bl_fwd(o,val,(A)); // emit all messages except [SYS:] msg's
    }
  }
